...
function HomeController ($scope, auth, Repositories) {
  Repositories.fetch().
    then(renderData).
    then(setDataSource).
    then(cacheData);
  ...
  if (auth && auth.github) {
    $scope.loggedInUser = auth.github.username;
  }
...
angular.module('ghIssuesApp').
  controller(
      'HomeController',
      ['$scope', 'auth', 'Repositories', HomeController]).
  filter('meFirst', [meFirstFilter]);
